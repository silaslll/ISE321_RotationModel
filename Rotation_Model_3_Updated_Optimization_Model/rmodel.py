...
##Currently works, doesnt match Shutians exact answer bc Dvac?
##Only running 1 yr rn bc commented others out
def main():
...
def getData(dict,c,con):    
    ...
    num_classes = 1 #5 #c.execute('SELECT number FROM class WHERE number IS NOT ""')
    ...
    residents = {'Koss', 'Krim', 'Leiv', 'Min', 'Tcho', 'Wong'} #["Sydney", "Tim", "Amanda"] #PGY5
    ...
    departments = ['Nights', 'HPB-Chabot', 'CR', 'HPB', 'Lap', 'Elective'] #["heart", "stomach", "ER", "ortho", "brain", "ear", "plastic"]
    ...
    # e = 5
        ...
        for r in residentsE[2]: 
            t_min[r,"Consults"] = 3
            t_max[r,"Consults"] = 3

        for r in residentsE[3]:
            t_max[r,'Trauma']= 2
            t_max[r,'CR']= 2
            t_max[r,'Breast']= 2
...
def constraints(m, t_min, t_max, r_min, r_max, Dvac, Tvac, Rvac, residentsE, blocksE, departmentsE, departmentsImp, departmentsReq, departmentsBusyE, blocksImp, weeksBE, dict,x,z,v):
    for e in dict["classes"]:
        ...
        ##constraint 1c, ensures each resident is assigned to required departments at least once
        ...
        con_dept_imp = m.addConstrs(z[r,d,b]==0 for r in residentsE[e] for d in departmentsE[e] if d in departmentsImp[r] for b in blocksE[e]) 
        ...
        con_no_vac_in_imp_blck = m.addConstrs(v[r,d,w]==0 for r in residentsE[e] for b in blocksImp[r] for w in dict["weeks"] for d in departmentsE[e])
        ...
        # print("Num of vacations in a week for a resident: ", (quicksum(v[r,d,w] for d in dict["departments"] for w in weeksBE[e,b]) for r in residentsE[e] for b in blocksE[e]))
        ##ensures that a resident can only take one vacation in a given week <- Came from Shutians model...not sure if I should include?
        # con_onevac= m.addConstrs(quicksum(v[r,d,w] for d in departmentsE[e] for w in dict["weeks"])<=1 for r in residentsE[e]) # for b in blocksE[e])
        con_onevac= m.addConstrs(quicksum(v[r,d,w] for d in dict["departments"])<=1 for w in dict["weeks"] for r in dict["residents"]) # for b in blocksE[e])
        ##something wrong here?
        # con_onevac= m.addConstrs(quicksum(v[r,d,w] for d in dict["departments"] for w in dict["weeks"])<=1 for r in residentsE[e] for b in blocksE[e])
        ...
        con_lim_res_vacs_in_block = m.addConstrs(quicksum(v[r,d,w] for w in weeksBE[e,b] for d in dict["departments"]) <= 1 for r in residentsE[e] for b in blocksE[e])
...
def solve(m):
    ...
    for v in m.getVars(): # A for loop iterates over the variables from the optimization model (from Gurobi) using the m.getVars() method.
        ...
        if v.x == 1: # When assign the resident to this rotation in the block 
            ...
            if v.varName[0] == 'v':
                # print(v.varName) 
                outputVac.append(re.split(',+', v.varName.strip(' v[]')))
    num = np.array(outputVac)
